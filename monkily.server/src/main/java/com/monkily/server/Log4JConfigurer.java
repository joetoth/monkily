...
public abstract class Log4JConfigurer {
	...
	/**
	 * Initialize Log4J with the given configuration and the default refresh
	 * interval. (org.apache.log4j.helpers.FileWatchdog.DEFAULT_DELAY, normally
	 * 60 seconds).
	 * 
	 * @param location
	...
	/**
	 * Initialize Log4J with the given configuration. Assumes an XML file in
	 * case of a ".xml" file extension.
	 * 
	 * @param location
	...
	public static void initLogging(String location, long refreshInterval)
		...
		if (!(new File(location)).exists()) {
			throw new FileNotFoundException("Log4j config file [" + location
					+ "] not found");
		}
	...
	public static void initLogging(Properties properties) {
	...
	/**
	 * Set the specified system property to the current working directory. This
	 * can be used e.g. for test environments, for applications that leverage
	 * Log4jWebConfigurer's "webAppRootKey" support in a web environment.
	 * 
	 * @param key
	 *            system property key to use
	 * @see org.springframework.web.util.Log4jWebConfigurer
	 */
